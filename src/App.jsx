...
export default function App() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (isAdmin(session)) {
        const tasksData = await listAllTasks();
...
function AffiliateTasksPage({ requests, setRequests, showToast }) {
    ...
    async function save(id, task) {
        ...
        if (patch.videoLink && patch.adCode) {
            patch.status = "Ad Code Submitted";
        } else if (patch.videoLink) {
            patch.status = "Video Submitted";
        }
            ...
            {tasks.map((t) => (
                <Card key={t.id} className="p-3">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                        <div className="font-semibold truncate flex-1" title={t.productTitle}>{t.productTitle}</div>
                    ...
                    <div className="mt-2 space-y-2">
                        <input
                            ...
                            className="w-full rounded-lg border border-white/20 bg-white/10 px-3 py-1.5 text-xs"
                        />
                        <input
                            value={t.adCode || ''}
                    ...
                    <div className="mt-2 flex justify-end">
                        <button onClick={() => save(t.id, t)} className="rounded-lg border border-indigo-400/50 bg-indigo-500/80 hover:bg-indigo-500 px-3 py-1 text-xs font-semibold transition-colors">
                            Submit
                        </button>
                    </div>
                </Card>
